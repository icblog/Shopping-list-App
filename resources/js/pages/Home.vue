      ...
      <div class="row">
        <div class="col-md-9 mx-auto">
          ...
          <section v-else>
            <h6>Add list name</h6>
            <p class="small">
              To create a new list, give it a name (for example, 'Weekly List' or 'Easter
              List') and click on it. Alternatively, you can select an existing list from
              the options below to add or edit items.
            </p>
            <ListNameForm />
            <hr class="m-0" />
            <div class="row">
              <div class="col">
                <p class="m-0 small bolded">Saved list</p>
              </div>
            ...
            <section v-if="reactiveData.data_length > 0">
              ...
              <div v-else class="mt-4">
                <div v-for="(list_name, index) in reactiveData.api_res" :key="index">
                  <div
                    :class="{
                      'data-detail-wrapper row': true,
                      'data-detail-wrapper-active': returnIfInputIsActive(list_name.id),
                    }"
                  >
                    <div class="col-7">
                      <div
                        v-show="!returnIfInputIsActive(list_name.id)"
                        class="link-wrapper bolded"
                      >
                        <Link :href="`/s-list/${list_name.name}/${list_name.id}`">{{
                          list_name.name
                        }}</Link>
                      </div>
                      <div
                    ...
                    <div class="col-5 text-right">
                      <span v-if="returnIfInputIsActive(list_name.id)">
                        <!-- Okay button -->
                        <AppButton
                          customClass="mr-2"
                          btnStyle="success"
                        ...
                        >
                          <span v-if="returnIfDataIsEditing(list_name.id)"
                            >Processing....</span
                          >
                          <span v-else-if="returnIfDataEditsuccess(list_name.id)">{{
                        ...
                        <AppButton
                          btnStyle="warn"
                          :hideBtn="hideCancelBtn(list_name.id)"
                      ...
                      <span v-else>
                        ...
                        <AppButton
                          ...
                          customClass="mr-2"
                          :hideBtn="
                            returnIfDataIsDeleting(list_name.id) ||
                            returnIfDataIsDeleteSuccess(list_name.id)
                          "
                          :btnFunc="
                            () => handleEditBtn(index, list_name.id, list_name.name)
                          "
                        >
                        ...
                        <AppButton
                          ...
                          "
                          :btnFunc="
                            () => handleDeleteDataYes(list_name.id, list_name.name)
                          "
                        >
                          <span v-if="returnIfDataIsDeleting(list_name.id)"
                            >Processing...</span
                          >

                          <span v-else-if="returnIfDataIsDeleteSuccess(list_name.id)">{{
                            reactiveData.edit_delete_success
                          }}</span>
                    ...
                    <div
                      ...
                      v-if="showErrMsgIfNotEmpty(list_name.id)"
                    >
              ...
              <div v-show="list_name_res?.next_page_url != null" class="pt-3">
...
const reactiveData = reactive({
  ...
  data_length:
    props.list_name_res.data.length == undefined ? 0 : props.list_name_res.data.length,
  current_records_on_page:
    props.list_name_res.data.length == undefined ? 0 : props.list_name_res.data.length,
  total_records: props.list_name_res.total == undefined ? 0 : props.list_name_res.total,
  main_err:
    props.list_name_res.data.error == undefined ? "" : props.list_name_res.data.error,
...
const assignDataFromAxiosApiCall = (is_paginate_result) => {
  reactiveData.data_length = props.list_name_res?.data.length;
  reactiveData.total_records = props.list_name_res?.total;
  //if paginate result, add else replace.
  if (is_paginate_result) {
    reactiveData.api_res = [...reactiveData.api_res, ...props.list_name_res?.data];
    reactiveData.current_records_on_page =
      reactiveData.current_records_on_page + props.list_name_res?.data.length;
  } else {
    reactiveData.api_res = props.list_name_res?.data;
    reactiveData.current_records_on_page = props.list_name_res?.data.length;
  }
};
...
const handleLoadMoreResult = async () => {
  if (!reactiveData.is_searching) {
    if (props.list_name_res?.next_page_url === null) {
      return;
    }
    // reactiveData.first_page_load = false;

    router.post(props.list_name_res?.next_page_url, returnDataToBePost(), {
      preserveState: true,
      preserveScroll: true,
      only: ["list_name_res"],
      onStart: () => {
      ...
      onSuccess: () => {
        ...
        window.history.replaceState({}, "", props.list_name_res?.path);
...
const makeApiCall = async () => {
  router.post("/", returnDataToBePost(), {
    onFinish: () => {
      if (reactiveData.is_searching) {
...
const handleDataUpdate = (index) => {
  ...
  if (!abort) {
    ...
    setTimeout(async () => {
      try {
        const res = await axios.post("/update-list-name", {
...
const handleDeleteDataYes = (data_id, data_name) => {
  ...
  if (
    window.confirm(
      `Delete list_name ${data_name}? please note this can not be undone, also all keys and key auth related to list_name ${data_name} will be deleted.`
    )
  ) {
    reactiveData.is_deleted = false;
    ...
    setTimeout(async () => {
      try {
        const res = await axios.post("/delete-list-name", {
          data_id: data_id,
        });
